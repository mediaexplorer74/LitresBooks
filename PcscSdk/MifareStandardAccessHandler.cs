...
namespace MifareStandard
{
    public class DefaultKeys
    {
        ...
        /*
        public static readonly byte[] KEY_A = 
        {
            0xA7,            0x3F,            0x5D,
            0xC1,            0xD3,            0x33
        };

        public static readonly byte[] KEY_B = 
        {
            0xE3,            0x51,            0x73,
            0x49,            0x4A,            0x81
        };

        public static readonly byte[] KEY_0 = 
        {
            0x00,            0x00,            0x00,
            0x00,            0x00,            0x00
        };
    ...
    public class AccessHandler
    {
        ...
        {
            var apduRes = await connectionObject.TransceiveAsync
            (new MifareStandard.LoadKey(mifareKey, keySlotNumber));

            if (!apduRes.Succeeded)
            {
                throw new Exception("Failure loading key for MIFARE Standard card, " 
                + apduRes.ToString());
            }
        ...
        (
            ushort blockNumber, 
            GeneralAuthenticate.GeneralAuthenticateKeyType keyType, 
        ...
        )
        {
            ...
            /*
            var genAuthRes = 
            await connectionObject.TransceiveAsync
            (
                new MifareStandard.GeneralAuthenticate(blockNumber, keySlotNumber, keyType)
            );
