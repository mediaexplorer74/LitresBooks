...
namespace NfcSample
{
    ...
    public sealed partial class CardReader : Page
    {
        ...
        private async Task HandleCard(SmartCard card)
        {
            try
            {
                ...
                using (SmartCardConnection connection = await card.ConnectAsync())
                {
                        ...
                        && cardIdentification.PcscCardName == Pcsc.CardName.FeliCa)
                    {
                        ...
                        && (cardIdentification.PcscCardName == Pcsc.CardName.MifareStandard1K 
                        || cardIdentification.PcscCardName == Pcsc.CardName.MifareStandard4K))
                    {
                        ...
                        for (ushort address = 0; address <= maxAddress; address++)
                        {
                            ...
                            var response = await mfStdAccess.ReadAsync
                                (address, 
                                Pcsc.GeneralAuthenticate.GeneralAuthenticateKeyType.MifareKeyA
                                );

                            // Message
                            LogMessage("Block " + address.ToString() + " " + 
            ...
            {
                LogMessage("Exception handling card: " + ex.Message);
                Debug.WriteLine("!!Exception!! : " + ex.Message);//("Exception handling card: " + ex.ToString(), NotifyType.ErrorMessage);
            }
        }
    }
}
